...
Page({
  ...
  data: {
    ...
    roomImages: [],   //已存在的房间图片
    addedRoomImages:[], //新添加的房间图片
    isSubmit:false,   //是否提交表单成功
    //roomId:""         //类型为change时，记录房间id
  },
  ...
  imagesEvent:function(e){
    ...
    var images = e.detail.imgUrl
    var num = images.length
    var addedRoomImages = that.data.addedRoomImages.concat(images)
    that.setData({
      addedRoomImages: addedRoomImages,
    })
  },
  //提交表单
  submitForm:function(e){
    ...
    //上传图片到云文件
    var filePath = that.data.addedRoomImages;
    console.log(filePath)
    for (var i in filePath) {
      var count = parseInt(that.data.roomImages.length+i)
      //获取数据库中图片数量count，通过“count.后缀”来添加图片保证图片不覆盖 + "/" +  
      var url = "images-roomType/" + app.globalData.openId + "/" + that.data.typeName + "/" + count + filePath[i].match(/\.[^.]+?$/)[0];
      //将addedRoomImage
    ...
    if(that.data.pageType == 'add'){
      ...
      db.collection('roomType').where({
        ...
        success(res){
          ...
          var images = that.data.roomImages.concat(that.data.addedRoomImages);
